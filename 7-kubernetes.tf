resource "google_service_account" "k8s-staging-mundo" {
  project    = local.service_project_id
  account_id = "k8s-staging-mundo"

  depends_on = [google_project.k8s-staging-mundo]
}
...
resource "google_container_node_pool" "general" {
  ...
  node_config {
    ...
    service_account = google_service_account.k8s-staging-mundo.email
    oauth_scopes = [
